...
class GetAllPotentialKits(TestCase):
    ...
    def test_dividebag_bag0_quantity_1(self):
        ...
        response = self.client.put(
            reverse(self.route), {"bag": self.bags[0].pk, "quantity": 1}
        )
        print(response.data)
        ...
        # Test models in DB
        old_bag = Bag.objects.get(pk=response.data[0]["id"])
        new_bag = Bag.objects.get(pk=response.data[1]["id"])

        self.assertEqual(old_bag.name, new_bag.name)
        self.assertEqual(old_bag.kind, new_bag.kind)
        self.assertEqual(old_bag.complete, new_bag.complete)
        self.assertEqual(old_bag.quantity, original_quantity - quantity)
        self.assertEqual(new_bag.quantity, quantity)
